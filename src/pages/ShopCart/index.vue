        ...
        <ul class="cart-list" v-for="cart in cartInfoList" :key="cart.id">
          ...
          <li class="cart-list-con5">
            <a
              ...
              class="mins"
              @click="handleChange('minus', -1, cart)"
              >-</a
            >
            <input
              autocomplete="off"
              ...
              class="itxt"
              :value="cart.skuNum"
              @change="handleChange('change', $event.target.value, cart)"
            />
            <a
              href="javascript:void(0)"
              class="plus"
              @click="handleChange('plus', 1, cart)"
              >+</a
            >
          </li>
          ...
          <li class="cart-list-con7">
            <a class="sindelet" @click="handleDelete(cart)">删除</a>
            <br />
            <a href="#none">移到收藏</a>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    async changeAllChecked(event) {
      ...
      try {
        await this.$store.dispatch("changeAllChecked", state);
        this.getData();
      } catch (error) {
        console.error(error.message);
      }
    },
    handleChange: throttle(async function (type, disNum, cart) {
      switch (type) {
        case "plus":
          disNum = 1;
          break;
